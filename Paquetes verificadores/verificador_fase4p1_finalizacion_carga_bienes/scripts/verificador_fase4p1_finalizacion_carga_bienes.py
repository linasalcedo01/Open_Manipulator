...
class verificar_paquetes_actividadguiada1(object):
    def __init__(self):
        try:
            ...
            self.status= rospy.Subscriber('/gamora/carga_bienes/status', GoalStatusArray, self.listen_status)
    ...
    def respuesta_retroalimentaciones(self):
        ...
        try:
            ...
            if respuesta == 'ok':
                #print(f"{self.desocupado, self.cargando, self.finalizado, self.peticion, self.ultimo_estado}")
                if self.desocupado==True and self.cargando==True and self.finalizado==True and self.peticion==True and self.finalizacion_exitosa==True:
