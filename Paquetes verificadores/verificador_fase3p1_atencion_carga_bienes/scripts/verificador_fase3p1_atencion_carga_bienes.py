...
import re
import rospy
from action_carga_bienes_manipulador.msg import carga_bienesActionGoal, carga_bienesActionFeedback, carga_bienesActionResult
...
class verificar_paquetes_actividadguiada1(object):
    def __init__(self):
        try:
            ...
            self.package_name = 'action_carga_bienes_manipulador'  #colocar aquí el nombre del paquete a analizar :v
            self.feedback= rospy.Subscriber('/gamora/carga_bienes/feedback', carga_bienesActionFeedback, self.listen_feedback)
            self.result= rospy.Subscriber('/gamora/carga_bienes/result', carga_bienesActionResult, self.listen_resultado)

            self.goal= rospy.Subscriber('/gamora/carga_bienes/goal', carga_bienesActionGoal, self.listen_goal)
            self.PAQUETE = 0.0
            ...
            self.cal_inicio_peticion = self.inicio_peticion()
            self.cal_respuesta_retroalimentaciones = self.respuesta_retroalimentaciones()
            
            

            self.PAQUETE = (self.cal_existencia_paquete + self.cal_existencia_directorio_launch + self.cal_existencia_archivos_launch_py + self.cal_permisos_ejecucion_py + self.cal_existencia_action + self.cal_inicio_peticion + self.cal_respuesta_retroalimentaciones)
        ...
        try:
            ...
            if os.path.exists(launch_dir_path) and os.path.isdir(launch_dir_path):
                if os.path.isdir(launch_dir_path):
                    if "client_carga_bienes.launch" in os.listdir(launch_dir_path) and "server_carga_bienes.launch" in os.listdir(launch_dir_path):
            ...
            if os.path.exists(scripts_dir_path) and os.path.isdir(scripts_dir_path):
                if os.path.isdir(scripts_dir_path):
                    if "client_carga_bienes.py" in os.listdir(scripts_dir_path) and "server_carga_bienes.py" in os.listdir(scripts_dir_path):
        ...
        try:
            ...
            if os.path.exists(launch_dir_path) and os.path.isdir(launch_dir_path):
                if os.path.isdir(launch_dir_path):
                    if "client_carga_bienes.launch" in os.listdir(launch_dir_path)and "server_carga_bienes.launch" in os.listdir(launch_dir_path):
                        self.existe_archivo_launch= True
                        client_path = os.path.join(launch_dir_path, "client_carga_bienes.launch")
                        if os.access(client_path, os.X_OK):
                            self.launch_ejecutable_client = True
                        else:
                            self.launch_ejecutable_client = False
                        server_path = os.path.join(launch_dir_path, "server_carga_bienes.launch")
                        if os.access(client_path, os.X_OK):
            ...
            if os.path.exists(scripts_dir_path) and os.path.isdir(scripts_dir_path):
                if os.path.isdir(scripts_dir_path):
                    if "client_carga_bienes.py" in os.listdir(scripts_dir_path) and "server_carga_bienes.py" in os.listdir(scripts_dir_path):
                        self.existe_archivo_py=True
                        server_path = os.path.join(scripts_dir_path, "client_carga_bienes.py")
                        if os.access(server_path, os.X_OK):
                            self.server_py_ejecutable = True
                        else:
                            self.server_py_ejecutable = False
                        client_path = os.path.join(scripts_dir_path, "server_carga_bienes.py")
                        if os.access(client_path, os.X_OK):
    ...
    def existencia_action(self):
        ...
        try:
            ...
            action_topics = [
                '/gamora/carga_bienes/cancel',
                '/gamora/carga_bienes/feedback',
                '/gamora/carga_bienes/goal',
                '/gamora/carga_bienes/result',
                '/gamora/carga_bienes/status'
            ]

            all_exist = all(topic in topic_names for topic in action_topics)

            if all_exist:
                print("5.Existencia de la acción /gamora/carga_bienes.(7 %)")
                self.cal_existencia_action = 0.07
            else:
                print("5.Existencia de la acción /gamora/carga_bienes.(0 %)")
        ...
        return self.cal_existencia_action
            ...
            # Callback para /gamora/carga_bienes/goal
    def listen_goal(self, msg):
        self.color = msg.goal.color
        self.forma = msg.goal.forma
        return self.color, self.forma
    
    def inicio_peticion(self):

        respuesta = input("Por favor lanza el cliente de la Acción, cuando lo hagas escribe ok: ").strip().lower()
        try:
            while respuesta != "ok": 
            ...
            if respuesta == 'ok':
                ...
                while not self.color or not self.forma:
                    tiempo_actual = rospy.get_time()
                    if (tiempo_actual - tiempo_inicio) > 3.0:
                        break  # Sale del bucle si no se recibieron los datos a tiempo
                if self.color and self.forma:
                    print("6. Verificación inicio de petición, (30 %)")
                    self.cal_inicio_peticion=0.30
                else:
                    print("6. Verificación inicio de petición, (0 %)")
                    self.cal_inicio_peticion=0.0
        
        except subprocess.TimeoutExpired:
            print("6. Verificación inicio de petición, (0 %)")
            self.cal_inicio_peticion=0.0
        return self.cal_inicio_peticion

    
    # Callback para /gamora/carga_bienes/feedback
    def listen_feedback(self, msg):
        ...
        if self.texto_feedback== 'Cargando':
            self.cargando=True
        if 'Petición' in self.texto_feedback.split() and 'de' in self.texto_feedback.split() and 'carga' in self.texto_feedback.split() and 'recibida' in self.texto_feedback.split():
            self.peticion = True
    ...
    def respuesta_retroalimentaciones(self):
        ...
        try:
            while respuesta != "ok": 
                respuesta = input("comando inválido, escribe 'ok' para continuar la verificación:").strip().lower()
            if respuesta == 'ok':
                print(f"{self.desocupado, self.cargando, self.finalizado, self.peticion}")
                if self.desocupado==True and self.cargando==True and self.finalizado==True and self.peticion==True:
                    print("7. Verificación respuesta a Acción y retroalimentaciones, (35 %)")
                    self.cal_respuesta_retroalimentaciones=0.35
                else:
...
def main():
    rospy.init_node('verificador_fase3p1_atencion_carga_bienes', anonymous=True)
