    ...
    def respuesta_retroalimentaciones(self):
        ...
        try:
            while respuesta != "ok": 
                respuesta = input("comando inválido, escribe 'ok' para continuar la verificación:").strip().lower()
            if respuesta == 'ok':
                #print(f"{self.desocupado, self.descargando, self.finalizado, self.peticion}")
                if self.desocupado==True and self.descargando==True and self.finalizado==True and self.peticion==True:
