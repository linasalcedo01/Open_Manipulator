...
from action_descarga_bienes_manipulador.msg import descarga_bienesAction, descarga_bienesFeedback, descarga_bienesResult
...
class ServidorRecogida:
    def __init__(self):
        ...
        rospy.init_node('server_descarga_bienes')
        ...
        if not self.setActuatorState(True):  # Llamada para habilitar los actuadores
            ...
            self.server = actionlib.SimpleActionServer('descarga_bienes', descarga_bienesAction, self.execute_cb, False)
    ...
    def execute_cb(self, goal):
        ...
        self.objeto_existe=False
        self.terminacion_tracking=False
        feedback = descarga_bienesFeedback()
        self.result = descarga_bienesResult()
...
#FORMA ------> PISO POS RECO
        ...
        if self.color or self.forma:
            ...
            feedback.estado_actual = f"Petición de descarga recibida .Forma: {self.forma}, Color: {self.color}"
            self.server.publish_feedback(feedback)
...
################################################################################################################################################################################################################################                
                ...
                if (self.forma =='circulo' or self.forma=='cuadrado' or self.forma=='triangulo') and (self.color=='azul' or self.color=='rojo' or self.color=='verde'):
                    feedback.estado_actual = "Descargando"
                    self.server.publish_feedback(feedback)
                    rospy.sleep(1)
...
####################################################################################################################################################################################3
                ...
                if (self.color=='azul' or self.color=='rojo' or self.color=='verde') and (self.forma !='circulo' or self.forma!='cuadrado' or self.forma!='triangulo'):
                    feedback.estado_actual = "descarndo"
                    self.server.publish_feedback(feedback)
                    rospy.sleep(1)
                    self.tracking_color= True
                    self.agarre_color=True
